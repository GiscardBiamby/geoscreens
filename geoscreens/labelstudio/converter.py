"""
This is a copy of:
~/anaconda3/envs/geoscreens/lib/python3.9/site-packages/label_studio_converter/converter.py

with some small modifications to fix bugs related to image path in `convert_to_coco()`.
"""
import io
import json
import logging
...
class Converter(object):
    ...
    def __init__(
        self, config, project_dir, output_tags=None, upload_dir=None, download_resources=True
    ):
    ...
    def convert(self, input_data, output_data, format, is_dir=True, **kwargs):
        ...
        if format == Format.JSON:
            ...
            items = (
                self.iter_from_dir(input_data) if is_dir else self.iter_from_json_file(input_data)
            )
    ...
    def _get_supported_formats(self):
        ...
        if not (
            "Image" in input_tag_types
            and ("RectangleLabels" in output_tag_types or "PolygonLabels" in output_tag_types)
            or "Rectangle" in output_tag_types
            and "Labels" in output_tag_types
        ...
        with io.open(output_file, "w", encoding="utf8") as fout:
            ...
            for item in item_iterator(input_data):
                filtered_output = list(
                    filter(lambda x: x[0]["type"].lower() == "labels", item["output"].values())
                )
                tokens, tags = create_tokens_and_tags(
                    text=item["input"][data_key], spans=next(iter(filtered_output), None)
                )
    ...
    def convert_to_coco(self, input_data, output_dir, output_image_dir=None, is_dir=True):
        ...
        for item_idx, item in enumerate(item_iterator):
            ...
            if self.download_resources and not os.path.exists(image_path):
                try:
                    image_path = download(
                        image_path,
                        output_image_dir,
    ...
    def convert_to_yolo(
        self, input_data, output_dir, output_image_dir=None, output_label_dir=None, is_dir=True
    ):
        ...
        for item_idx, item in enumerate(item_iterator):
            ...
            label_path = os.path.join(
                output_label_dir, os.path.splitext(os.path.basename(image_path))[0] + ".txt"
            )
            annotations = []
            for label in labels:
        ...
        for name, info in self._schema.items():
            ...
            for label in attrs:
                if attrs[label].get("category"):
                    categories.append({"id": attrs[label].get("category"), "name": label})
