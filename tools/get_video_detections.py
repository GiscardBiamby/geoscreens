...
def segment_video(
    ...
    print("Sample_every_seconds: ", sample_every_seconds)
...
def make_dets_df(cats: Dict, frame_detections: Dict):
...
def get_categories():
    data = json.load(open(PROJECT_ROOT / "datasets/geoscreens_001/geoscreens_001.json", "r"))
...
def generate_detections(args, split: str):
    ...
    meta_data = [
        ...
        if (i % args.num_devices == args.device) and s["id"] in id_list
    ]
    config, module, model, light_model, geoscreens_data = get_model(args)

    print(f"Found {len(meta_data)} videos to segment")
    for video_info in tqdm(meta_data, total=len(meta_data), desc=f"segment_{split}_vids"):
        # video_id = "178LA7LXoCs"
        video_id = video_info["id"]
        csv_path = args.save_dir / split / f"df_frame_dets-video_id_{video_id}.csv"
        if csv_path.exists():
        ...
        # backaround
        cats = get_categories()
        df_frame_dets = make_dets_df(cats, frame_detections)
...
if __name__ == "__main__":
    ...
    generate_detections(args, "train")
